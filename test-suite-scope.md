...
Planned mapping: mirror TC-M-001–003 (smoke) + TC-M-004–008 (regression) as automated UI specs.

## API automation (Playwright — planned)
...
Planned coverage: login/token, create cart, GET products, invoice POST chain (smoke); register, cart mutations, token/payload negatives (regression).
